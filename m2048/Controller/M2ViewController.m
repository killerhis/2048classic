...
- (void)viewDidLoad
{
  ...
  _scene.controller = self;
    ...
    _bannerView.adUnitID = @"ca-app-pub-2660521344509391/8696587807";
...
- (GADInterstitial *)createAndLoadInterstitial
{
    ...
    interstitial.adUnitID = @"ca-app-pub-2660521344509391/1173321003";
...
#pragma mark - Game Viewcontroller
...
- (void)updateScore:(NSInteger)score
{
    ...
    if ([[GameCenterManager sharedManager] checkGameCenterAvailability]) {
        if (highScore < [[GameCenterManager sharedManager] highScoreForLeaderboard:@"2048_leaderboard"]) {
            highScore = [[GameCenterManager sharedManager] highScoreForLeaderboard:@"2048_leaderboard"];
            [Settings setInteger:highScore forKey:@"Best Score"];
        }
    }
...
- (void)endGame:(BOOL)won
{
    ...
    // GA
    id<GAITracker> tracker = [[GAI sharedInstance] defaultTracker];
    [tracker set:kGAIScreenName value:@"GameOver Scene"];
    [tracker send:[[GAIDictionaryBuilder createAppView] build]];
