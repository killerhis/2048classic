...
- (void)ratePopup
{
    self.rateView = [[UIView alloc] initWithFrame:[[UIScreen mainScreen] applicationFrame]];
    self.rateView.alpha = 0.0f;
    [self.view addSubview:self.rateView];
    
    UIButton *rateBackgroundButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [rateBackgroundButton setBackgroundColor:[UIColor blackColor]];
    [rateBackgroundButton addTarget:self action:@selector(closeRate) forControlEvents:UIControlEventTouchUpInside];
    rateBackgroundButton.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height);
    rateBackgroundButton.alpha = 0.5;
    [self.rateView addSubview:rateBackgroundButton];
    ...
    UIImage *image = [UIImage imageNamed:@"ratebut.png"];
    [self.rateButton setBackgroundImage:image forState:UIControlStateNormal];
    self.rateButton.frame = CGRectMake((self.view.frame.size.width - (image.size.width))/2, (self.view.frame.size.height - (image.size.height))/2, image.size.width, image.size.height);
    [self.rateView addSubview:self.rateButton];
    
    [UIView animateWithDuration:0.5f animations:^{
        [self.rateView setAlpha:1.0f];} completion:nil];
}

- (void)closeRate
{
    [UIView animateWithDuration:0.5f animations:^{
        [self.rateView setAlpha:0.0f];
...
- (void)rateApp
{
    ...
    NSString *str = @"http://itunes.apple.com/WebObjects/MZStore.woa/wa/viewContentsUserReviews?id=945818422&pageNumber=0&sortOrdering=2&type=Purple+Software&mt=8";
